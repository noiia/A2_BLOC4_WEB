...
return function (App $app) {
    ...
    $app->group('/', function ($group) {
        $group->get('a', function ($request, $response) {
            $response->getBody()->write("error logged");
            return $response;
        });
        ...
        $group->get('/Stage', function (Request $request, Response $response) use ($twig, $container) {
            $controller = new InternshipController($twig);
            $welcomeResponse = $controller->Welcome($request, $response, [], $container);
            return $welcomeResponse;
        });
        $group->get('/Stage/{id}', function (Request $request, Response $response, int $id) use ($container) {

            $entityManager = $container->get(EntityManager::class);
            $internship = $entityManager->getRepository(Internship::class)->findOneBy(['ID_Internship' => $id]);
            $i = 0;
            $Skills = [];
        ...
        $group->get('/Entreprise', function (Request $request, Response $response, array $args) use ($twig, $container) {
            $controller = new CompanyController($twig);
            $companyResponse = $controller->Company($request, $response, [], $container);
            return $companyResponse;
        });
        $group->get('/Entreprise/api/{id}', function (Request $request, Response $response, array $args) use ($container) {
        ...
        $group->post('/Entreprise/addComment', function (Request $request, Response $response, array $args) use ($twig, $container) {
            try {
                $controller = new CompanyController($twig);
        ...
        $group->get('/Statistique/Entreprises', function (Request $request, Response $response, array $args) use ($twig, $container) {
            try {
                $controller = new CompanyController($twig);
                $companyResponse = $controller->Company($request, $response, [], $container);
                return $companyResponse;
